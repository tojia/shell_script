...
do
    ...
    case ${anum} in
        ...
        4) echo -e "查看当前目录下每个文件夹的情况:du --max-depth=1 -h\n"
            ...
            # exit
            ;;
        5) echo "查看当前目录整体占用大小:du -sh\n"
            echo -e "`du -sh`"
            # exit
            ;;
        6) echo "查看指定目录下每个文件夹的情况:du --max-depth -h /xx/xx/xx"
            echo -e "您想查看哪个目录下的每个文件夹的占用情况？\n"
            read path
            echo -e "`du --max-depth=1 -h ${path}`"
            # exit
            ;;
        7) echo "查看指定目录的整体占用大小:du -sh /xx/xx/\n"
